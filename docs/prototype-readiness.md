...
| Measured evaluation evidence | Backend runs 30 deterministic acceptance controls and in-process engine percentiles; the benchmark separately measures real HTTP round-trip p50/p95/p99 under concurrent load, a concurrent budget race, and audit integrity | `test_benchmark_is_reproducible_and_safe`, `test_authorization_probe_uses_isolated_full_policy_path` |
| Adversarial resilience | Seven attack classes are driven against the real engine and the real gateway: intent tampering, replay after revocation, budget time-of-check/time-of-use, a 50-iteration concurrent spend race, fleet-epoch bypass, self-reported risk, and audit-chain mutation, deletion, reordering, and truncation | `tests/test_adversarial.py` |
